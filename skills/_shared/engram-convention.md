...
type:      architecture
project:   omit to use the MCP auto-detected current project
scope:     project
```
...
Engram auto-detects the project name from the git remote at MCP startup. The `--project` flag and `ENGRAM_PROJECT` env var can override detection, but agents SHOULD NOT force them in normal operation. Prefer automatic detection and omit the `project` field in `mem_*` calls unless you intentionally need cross-project access. All project names are normalized to lowercase and trimmed.
